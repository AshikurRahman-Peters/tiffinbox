...
function subjectSelection(){
	...
	$.post( "<?php echo base_url()?>examresult/getStudent", { subject_id:subject_id,exam_id: exam_id,class_id:class_id }).done(function( data ) {
...
function upadateStdResult(id,Std_id){
	...
	if(id==0){
		...
		$.post( "<?php echo base_url()?>examresult/insertStdResult", { exam_id: exam_id, class_id: class_id, subject_id:subject_id, Std_id: Std_id, result_id: id, marks: marks, attend: attend, comment: comment  }).done(function( data ) {
		  	 if(data != 0){
		  	 	alert ("Successfully inserted the result!");
			...
			alert ("Successfully update the result!");
			...
			<!--PAGE CONTENT BEGINS-->
					...
					<div class="row-fluid">
						...
						<?php
							...
							endforeach;
							echo '<span id="exam">'.form_dropdown('exam',$exam,'','id="eaxmSelection" onchange="examSelection()"').'</span>';
							...
							endforeach;
							echo '<span id="classes" style="display:none">'.form_dropdown('class',$classes,'',' id="classSelection" onchange="classSelection()"').'</span>';
						?>
						<span id="Subject" class="pull"></span>	
						<div id="Student"></div>	
							<?php echo form_close(); ?>
